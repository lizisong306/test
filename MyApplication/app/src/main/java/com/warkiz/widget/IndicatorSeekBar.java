...
public class IndicatorSeekBar extends View {
    ...
    private CharSequence[]  mTickTextskedu;
    //indicator
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mTickTextskedu = ta.getTextArray(R.styleable.IndicatorSeekBar_isb_tick_texts_kedu_array);
...
//            Log.d("lizisong", "mTickMarksX[i]:"+mTickMarksX[i]);
        }
    }
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            boolean state = false;
            int pro = (int)Math.round(mProgress);
            int curr = Integer.parseInt(mTickTextskedu[i].toString());
            if(curr >=pro-3 && curr <= pro+3){
                state=true;
            }
//            Log.d("lizisong","Math."+(int)Math.round(mProgress) +"===="+mTickTextskedu[i].toString());
            if ((i == getThumbPosOnTick() && i == thumbPosFloat) ||state) {
...
//            Log.d("lizisong", "mTickTextsArr[index]:"+mTickTextsArr[index]);
        }
    }

    public void drawCurrentTick(Canvas canvas){
