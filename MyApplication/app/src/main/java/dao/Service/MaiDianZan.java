...
/**
 * Created by 13520 on 2017/1/12.
 */

public class MaiDianZan {
    private static MaiDianZan instance;

    private Cpllecthelp dbHelper;

    public MaiDianZan (Context context){
     ...
     * @return
     */
    public static  MaiDianZan getInstance(Context context){
        if(instance == null && context == null){
            throw new IllegalArgumentException("Argument context can't be null!!!");
        }else if(instance == null){
            instance = new MaiDianZan(context);
        }
        return instance;
    }
    ...
    public synchronized long insert(CollectionEntity entity){
        ...
        long id = writeDatabase.insert("maidianzan", "id", values);
    ...
    public synchronized long delete(int id){
        SQLiteDatabase writeDatabase = dbHelper.getWritableDatabase();
        int num = writeDatabase.delete("maidianzan",  "id = ?", new String[] {
                String.valueOf(id)
        });
    ...
    public synchronized long deletebyaid(String aid){
        ...
        SQLiteDatabase writeDatabase = dbHelper.getWritableDatabase();
        int num = writeDatabase.delete("maidianzan",  "aid = ?", new String[] {
                aid
        });
    ...
    public long updata(CollectionEntity entity){
        ...
        values.put("iscollect",entity.iscollect);
        values.put("isAdd",entity.isAdd);
        int num = writeDatabase.update("maidianzan", values, "id = ?", new String[] {
    ...
    public ArrayList<CollectionEntity> getcollect(){
        ...
        Cursor cursor = readableDatabase.rawQuery("SELECT * FROM maidianzan  group by aid ORDER BY id  DESC"
                , null);
